...
interface WebSource { title: string; url: string; description: string; content: string; retrieved_at: string }
...
export function ChatB2KStreamPanel() {
  ...
  const { pushChunk, clearBuffer } = useStreamBuffer(useCallback((chunk: string) => {
    const decoded = decodeStreamChunk(chunk); if (!decoded) return;
    assistantTextRef.current += decoded;
    const id = assistantIdRef.current; if (!id) return;
  ...
  async function sendMessage(event: FormEvent) {
    ...
    assistantIdRef.current = assistantMessage.id; setMessages((current) => [...current, userMessage, assistantMessage]);
    ...
    setStreaming(true); setWebSearching(true);
    let webContext: WebSource[] | undefined;
    try {
      const webResponse = await fetch("/api/public/firecrawl", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ query: content }),
      });
      if (webResponse.ok) {
        const result = (await webResponse.json()) as { used?: boolean; sources?: WebSource[] };
    ...
    await startStream({
      ...
      onChunk: pushChunk,
      onComplete: async () => {
        clearBuffer(); const assistantContent = assistantTextRef.current.trim();
        if (assistantContent) {
  ...
  return <section className="glass rounded-md p-6 lg:col-span-3">
    ...
    <div className="mt-5 min-h-40 max-h-80 overflow-y-auto space-y-3 rounded-sm border border-[var(--glass-border)] bg-[var(--ink)] p-4">{messages.length === 0 ? <p className="text-sm text-muted-foreground">Ask ChatB2K what your next best action should be.</p> : messages.map((message) => <div key={message.id} className={message.role === "user" ? "ml-auto max-w-[85%] rounded-sm bg-gold-gradient p-3 text-sm text-[var(--ink)]" : "max-w-[85%] rounded-sm border border-[var(--glass-border)] p-3 text-sm text-foreground/90"}>{message.content || (streaming && message.id === assistantIdRef.current ? "…" : "")}</div>)}</div>
